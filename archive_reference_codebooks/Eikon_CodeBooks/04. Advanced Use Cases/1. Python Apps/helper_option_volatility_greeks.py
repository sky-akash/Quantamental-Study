...
try:
    rd.open_session( # For more info on the session, use `rd.get_config().as_dict()`
...
class get_options_RIC():
    ...
    def get_ric_ieu(self, asset, maturity, strike, opt_type, debug):
        ...
        for gen in generations:
            ...
            if len(prices) == 0:
                if debug:
                    print(f'Constructed {ric} RIC with specified parameters is not found')
            else:
                return ric, prices
        return  ric, prices
...
class IPA_Equity_Vola_n_Greeeks():
    ...
    def initiate(
        ...
        if self.debug:
            print(f"undrlying_mrkt_pr_gmt = rd.content.historical_pricing.summaries.Definition(universe='{self.underlying}', start='{df_strt_dt_str}', end='{df_end_dt_str}', interval='{self.data_retrieval_interval}', fields='{optn_mrkt_pr_gmt.columns.array[0]}').get_data().data.df")
            print(f"optn_mrkt_pr_gmt.columns.array: {optn_mrkt_pr_gmt.columns.array}")
    ...
    def get_data(self):       
        ...
        rf_rate_prct = self.get_history_mult_times(
            ...
            (self.df_strt_dt - timedelta(days=1)).strftime('%Y-%m-%d'),  # https://teamtreehouse.com/community/local-variable-datetime-referenced-before-assignment
        ...
        if self.debug:
            print("\n")
            print("self.df_gmt['RfRatePrct'][self.df_gmt.isna().any(axis=1)] : ")
            print("\n")
